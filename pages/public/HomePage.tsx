...
import { APP_NAME, MOCK_BARBERSHOP_ID, PREDEFINED_REVIEWS, MOCK_SERVICES_DATA, ROUTES } from '../../constants'; // Added ROUTES
import { Scissors, Star, CalendarPlus } from 'lucide-react';
import Card from '../../components/ui/Card';
import StarRating from '../../components/ui/StarRating';

const HomePage: React.FC = () => {
  const featuredServices = MOCK_SERVICES_DATA.slice(0, 4); // Show first 4 services as example
  const featuredReviews = PREDEFINED_REVIEWS.slice(0, 3); // Show first 3 reviews

  return (
    <div className="text-branco-nav">
      {/* Hero Section */}
      <section className="relative py-20 md:py-32 bg-cover bg-center bg-no-repeat bg-leather-texture" style={{ backgroundImage: "url('https://picsum.photos/seed/barberhero/1920/1080'), linear-gradient(rgba(13,31,45,0.8), rgba(13,31,45,0.8))", backgroundBlendMode: 'overlay' }}>
        <div className="absolute inset-0 bg-azul-marinho opacity-70"></div>
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center z-10">
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-roboto-slab font-bold mb-6">
            Bem-vindo ao <span className="text-vermelho-bordo">{APP_NAME}</span>
          </h1>
            ...
            <Link to={`${ROUTES.BARBERSHOP_PUBLIC}/${MOCK_BARBERSHOP_ID}`}>
             ...
             <Link to={ROUTES.REGISTER}>
              <Button variant="outline" size="large" className="w-full sm:w-auto">
                <Scissors size={20} className="mr-2" /> Sou Dono de Barbearia
              </Button>
            </Link>
          </div>
        </div>
      ...
      <section className="py-16 md:py-24 bg-azul-marinho">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8 text-center">
            <Card className="hover:shadow-vermelho-bordo/30 transition-shadow">
              <Scissors size={48} className="mx-auto mb-4 text-vermelho-bordo" />
            ...
            <Card className="hover:shadow-vermelho-bordo/30 transition-shadow">
              <CalendarPlus size={48} className="mx-auto mb-4 text-vermelho-bordo" />
            ...
            <Card className="hover:shadow-vermelho-bordo/30 transition-shadow">
              <Star size={48} className="mx-auto mb-4 text-vermelho-bordo" />
              <h3 className="text-xl font-roboto-slab font-semibold mb-2">Estilo e Profissionalismo</h3>
              <p className="text-gray-400">Design retrô moderno que reflete a identidade da sua barbearia premium.</p>
            </Card>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 md:py-24 bg-gray-800 bg-opacity-30">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-roboto-slab font-bold text-center mb-12">Serviços Populares</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {featuredServices.map(service => (
              <Card key={service.id} className="text-center bg-azul-marinho hover:border-vermelho-bordo transition-colors">
                 <Scissors size={32} className="mx-auto mb-3 text-vermelho-bordo" />
                <h3 className="text-lg font-roboto-slab font-semibold mb-1">{service.name}</h3>
                <p className="text-vermelho-bordo font-semibold mb-2">R$ {service.price.toFixed(2)}</p>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {featuredReviews.map((review, index) => (
              <Card key={index} className="flex flex-col bg-gray-800 bg-opacity-40">
                <div className="flex-grow">
                  <StarRating rating={review.rating} readOnly={true} size={18} className="mb-3" />
      ...
      <section className="py-16 md:py-24 bg-vermelho-bordo text-branco-nav">
